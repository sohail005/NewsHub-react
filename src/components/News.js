...
export default class News extends Component {
    ...
    constructor() {
        super();
        console.log("Hey i am a constructor from new component");
        this.state = {
            articles: [],
            loading: false,
            page: 1,
    ...
    handlePreviousClick = async () => {
        this.setState({ loading: true })
        let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=6a88f568c476465fb5df5c77a31488b7&page=${this.state.page - 1}&pageSize=${this.props.pageSize}`;
        let data = await fetch(url);
        let parsedData = await data.json();
        console.log(parsedData);
        this.setState({
    ...
    handleNextClick = async () => {
        console.log("this.state.totalResults:", this.state.page + 1 > Math.ceil(this.state.totalResults / 20));
